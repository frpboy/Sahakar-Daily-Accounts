    ...
    <Container className="py-10 max-w-7xl">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-12 border-b border-gray-200 pb-8">
          <div className="space-y-1">
            <h1 className="text-4xl font-black text-gray-900 tracking-tighter uppercase italic">System Outlets</h1>
          ...
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            ...
            <DialogContent className="max-w-md bg-white border border-gray-200 shadow-none rounded-none p-10 focus-visible:outline-none">
              <DialogHeader className="space-y-2 text-left">
                <DialogTitle className="text-2xl font-black text-gray-900 tracking-tighter uppercase italic">Registration</DialogTitle>
                <DialogDescription className="text-[11px] font-bold text-gray-400 uppercase tracking-widest leading-relaxed">
                  Enterprise branch provisioning. Automated ID generation activated.
                </DialogDescription>
              </DialogHeader>
              <form onSubmit={handleCreateOutlet} className="space-y-8 pt-6">
                <div className="space-y-5">
                  <div className="space-y-2">
                    ...
                    <Select value={newType} onValueChange={(v: any) => setNewType(v)}>
                      ...
                      <SelectContent className="rounded-none border-gray-200">
                        <SelectItem value="Pharmacy" className="text-xs font-bold">HYPER PHARMACY (SHP)</SelectItem>
                        <SelectItem value="Clinic" className="text-xs font-bold">SMART CLINIC (SSC)</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                  <div className="space-y-2">
                    <Label className="text-[10px] font-black uppercase text-gray-500 tracking-widest">Outlet Name</Label>
                    <Input 
                  ...
                  <div className="space-y-2">
                    <Label className="text-[10px] font-black uppercase text-gray-500 tracking-widest">Location Code/Name</Label>
                    <Input 
        ...
        {isDataLoading ? (
          <div className="flex justify-center py-32 border border-dashed border-gray-200">
             <div className="space-y-5 text-center">
                ...
                <p className="text-[10px] font-black uppercase tracking-[0.3em] text-emerald-600">Syncing Intelligence</p>
             </div>
          </div>
        ) : (
          <div className="grid gap-0 border-l border-t border-gray-200 md:grid-cols-2 lg:grid-cols-3">
            {outlets.map((outlet) => (
              <Link key={outlet.id} href={`/outlets/${outlet.id}`} className="group">
                <div className="h-full border-r border-b border-gray-200 bg-white hover:bg-gray-50 transition-all duration-300 p-8 flex flex-col justify-between">
                  <div className="space-y-6">
                    ...
                    <div className="space-y-1">
                      <h3 className="text-xl font-black tracking-tighter text-gray-900 uppercase italic leading-tight">{outlet.name}</h3>
                      <div className="flex items-center gap-1.5">
                        <Hash className="h-3 w-3 text-emerald-500" />
                        <span className="text-[9px] font-bold text-gray-400 uppercase tracking-widest">{outlet.entriesCount} ACTIVE DATA POINTS</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mt-10 space-y-4">
                    <div className="grid grid-cols-2 border border-gray-200 divide-x divide-gray-200">
                        <div className="p-4 space-y-1 bg-gray-50/50">
                           ...
                           <p className="text-sm font-black font-mono text-gray-900">{formatCurrency(outlet.totalSales)}</p>
                        </div>
                        <div className="p-4 space-y-1">
                           <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Net Index</p>
                           <p className={`text-sm font-black font-mono ${outlet.totalSales - outlet.totalExpenses >= 0 ? "text-emerald-600" : "text-red-600"}`}>
                              {formatCurrency(outlet.totalSales - outlet.totalExpenses)}
                           </p>
                        </div>
                    </div>

                    <div className="flex items-center justify-end group/btn pt-2 cursor-pointer">
                      <span className="text-[9px] font-black uppercase tracking-widest text-gray-400 group-hover/btn:text-gray-900 transition-colors mr-2">Access Ledger</span>
