...
export default function AdminView() {
  ...
  const [creationModal, setCreationModal] = useState<{ type: "Hyper Pharmacy" | "Smart Clinic"; isOpen: boolean }>({
    type: "Hyper Pharmacy",
    isOpen: false,
  });
  ...
  const handleExportCSV = () => {
    const exportData = stats.outletsStatus.map(outlet => ({
      ...
      Time: outlet.submittedAt ? new Date(outlet.submittedAt).toLocaleString() : "N/A"
    }));
    exportToCSV(exportData, `Sahakar_Outlet_Status_${new Date().toISOString().split('T')[0]}`);
  };

  const handleExportPDF = () => {
    exportToPDF("submission-tracker-table", "Sahakar Outlet Submission Tracker");
  };
  ...
  return (
    <Container className="py-8 space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
          <h1 className="text-3xl font-black text-gray-900 tracking-tighter uppercase">Admin Console</h1>
          <p className="text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em] mt-1">Enterprise-wide performance and system oversight</p>
        </div>
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
           ...
           <DropdownMenu>
             <DropdownMenuTrigger asChild>
               <Button className="h-10 px-6 font-black text-[10px] uppercase tracking-widest shadow-none">
                 <Plus className="mr-2 h-3 w-3" /> Create Outlet <ChevronDown className="ml-2 h-3 w-3 opacity-50" />
               </Button>
             </DropdownMenuTrigger>
             <DropdownMenuContent align="end" className="w-56">
               <DropdownMenuItem 
                 onClick={() => setCreationModal({ type: "Hyper Pharmacy", isOpen: true })}
               ...
               <DropdownMenuItem 
                 onClick={() => setCreationModal({ type: "Smart Clinic", isOpen: true })}
                 className="flex items-center gap-3 p-3 font-bold text-[10px] uppercase tracking-wider"
               >
                 <div className="h-6 w-6 bg-gray-900 text-white rounded-none flex items-center justify-center">
           ...
           <Link href="/admin/users" className="hidden lg:block">
             <Button variant="outline" className="h-10 px-6 font-black text-[10px] uppercase tracking-widest">
        ...
        <CardHeader className="bg-gray-50/50 border-b py-3 px-6">
           <CardTitle className="text-[10px] font-black uppercase tracking-[0.2em] flex items-center justify-between text-gray-500">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[9px] font-bold text-gray-400 opacity-50 px-2 py-0.5 border border-gray-200">LIVE</span>
              </div>
              <div className="flex gap-2">
                <Button variant="ghost" size="sm" onClick={handleExportCSV} className="h-7 text-[9px] font-black uppercase tracking-widest text-gray-400 hover:text-gray-900 border border-gray-100 bg-white px-3">
                  <FileText className="h-3 w-3 mr-1.5" /> CSV
                </Button>
                <Button variant="ghost" size="sm" onClick={handleExportPDF} className="h-7 text-[9px] font-black uppercase tracking-widest text-gray-400 hover:text-gray-900 border border-gray-100 bg-white px-3">
                ...
                <Link key={outlet.id} href={`/outlets/${outlet.id}`}>
                  <div className="flex flex-col gap-2 p-3 border border-gray-100 rounded-none bg-white hover:border-gray-900 transition-all group cursor-pointer h-full">
                    <div className="flex justify-between items-center">
                      <div className={`h-1.5 w-1.5 rounded-none ${outlet.isSubmitted ? "bg-emerald-500" : "bg-red-500"}`} />
                      <p className="text-[9px] font-black text-gray-400 group-hover:text-gray-900 transition-colors uppercase tracking-widest">{outlet.code}</p>
                    </div>
                    <div>
      ...
      <Card className="overflow-hidden">
        <CardHeader className="py-3 px-6 border-b bg-gray-50/50">
           <CardTitle className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-500">Activity Trail</CardTitle>
        </CardHeader>
             ...
             <Table>
                <TableHeader>
                   <TableRow>
                      ...
                      <TableHead>Entity Pointer</TableHead>
                   </TableRow>
                </TableHeader>
                <TableBody>
                   {stats.auditLogs.map((log, i) => (
                     <TableRow key={i}>
                        <TableCell className="text-gray-400 font-mono text-[10px] whitespace-nowrap">{formatDistanceToNow(log.time)}</TableCell>
                        <TableCell className="font-bold text-gray-900 text-xs uppercase tracking-tight">{log.user}</TableCell>
                        <TableCell>
                           <span className="px-1.5 py-0.5 bg-gray-900 text-white rounded-none text-[8px] font-black uppercase tracking-widest">{log.action}</span>
                        </TableCell>
                        <TableCell className="text-gray-500 font-mono text-[10px] truncate max-w-[200px] uppercase">{log.outlet}</TableCell>
                     </TableRow>
                   ))}
                </TableBody>
             </Table>
      ...
      <CardContent className="p-6">
        <div className="flex flex-col gap-5">
          <div className="flex items-center justify-between">
            ...
            <div className={`p-2 ${bg} rounded-none`}>
              {icon}
            </div>
          </div>
          <div>
            <p className={`text-2xl font-black tracking-tighter font-mono ${isSuccess ? 'text-emerald-500' : isDanger ? 'text-red-500' : 'text-gray-900'}`}>
              {value}
            </p>
            <p className="text-[9px] font-black text-gray-300 uppercase tracking-widest mt-1.5">{sub}</p>
          </div>
        </div>
      </CardContent>
    </Card>
