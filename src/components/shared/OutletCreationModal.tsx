...
export function OutletCreationModal({ type, isOpen, onClose, onSuccess }: OutletCreationModalProps) {
        ...
        <form onSubmit={handleSubmit}>
          <DialogHeader className="flex flex-col items-center text-center space-y-4 mb-8">
            <div className="h-20 w-20 bg-gray-900 text-white rounded-none flex items-center justify-center shadow-lg">
              {type === "Hyper Pharmacy" ? <Building2 className="h-10 w-10" /> : <Stethoscope className="h-10 w-10" />}
            </div>
            <div className="space-y-1">
              <p className="text-[10px] font-black uppercase text-gray-400 tracking-[0.3em] leading-none">Terminal Initialization</p>
              <DialogTitle className="text-2xl font-black text-gray-900 tracking-tighter uppercase">New {type}</DialogTitle>
            </div>
          </DialogHeader>

          <div className="space-y-8">
            <div className="space-y-2">
              <Label htmlFor="name" className="text-[10px] font-black text-gray-400 uppercase tracking-widest pl-0">Branch Descriptor</Label>
              <Input
                id="name"
                value={name}
            ...
            <div className="space-y-2">
              <Label htmlFor="location" className="text-[10px] font-black text-gray-400 uppercase tracking-widest pl-0">Geographic Pointer</Label>
              <Input
                id="location"
                value={location}
            ...
            <div className="bg-gray-50 rounded-none p-5 border border-gray-100 flex items-center justify-between">
                <div>
                   <p className="text-[9px] font-black text-gray-400 uppercase tracking-[0.2em] mb-1">System Allocation Hash</p>
                   <p className="font-mono text-xs font-black text-gray-900 tracking-[0.2em]">{type === "Hyper Pharmacy" ? "SHP-###" : "SSC-###"}</p>
                </div>
                <div className={`h-2.5 w-2.5 ${type === "Hyper Pharmacy" ? "bg-gray-900" : "bg-gray-400"} animate-pulse`} />
            </div>
          </div>
