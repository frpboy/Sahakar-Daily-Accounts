...
export function PWAHandler() {
  ...
  const handleInstallClick = async () => {
    ...
    if (outcome === "accepted") {
      ...
      notifySuccess("Thank you for installing Sahakar ERP!");
    } else {
      setShowInstallDialog(false);
    }
  };
  ...
  const handleNotificationRequest = async () => {
    ...
    if (permission === "granted") {
      notifySuccess("Notifications enabled successfully!");
    } else {
        ...
        <DialogContent className="sm:max-w-md bg-white rounded-3xl border-none shadow-2xl p-8">
          <DialogHeader className="flex flex-col items-center text-center space-y-4">
            ...
            <div className="space-y-1">
              <DialogTitle className="text-2xl font-black text-gray-900 tracking-tighter">
                Install Sahakar ERP
              </DialogTitle>
              <DialogDescription className="text-gray-500 font-medium leading-relaxed">
                Experience Sahakar ERP with full offline capabilities, faster
                load times, and real-time data synchronization.
                <br />
                <br />
                <span className="text-xs font-black uppercase text-gray-400 tracking-widest block mb-1">
                  How to Install
                </span>
                • On Chrome: Just click &quot;Install App Now&quot;
                <br />• On iOS: Tap the <span className="font-bold">
                  Share
                </span>{" "}
                button and select{" "}
          ...
          <DialogHeader className="flex flex-col items-center text-center space-y-4">
            ...
            <div className="space-y-1">
              ...
              <DialogDescription className="text-gray-500 font-medium">
                Stay updated with critical alerts, branch status, and account
                updates in real-time. We promise to only send important info.
              </DialogDescription>
            </div>
          </DialogHeader>
          <DialogFooter className="flex flex-col sm:flex-row gap-3 pt-4 sm:justify-center">
